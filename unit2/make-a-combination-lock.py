# A combination lock graph on n nodes consists of the edges
# in a chain AND the edges in a star, with the left end of
# the chain as the center of the star.
#
...
def create_combo_lock(nodes):
    G = {}
    midnodes = [n for n in nodes if n > 1 and n < len(nodes) - 1]

    for i in nodes:
        make_link(G, i, (i+1) % len(nodes))
        if i in midnodes:
            make_link(G, nodes[0], i)
