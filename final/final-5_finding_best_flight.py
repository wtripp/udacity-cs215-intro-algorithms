...
# Example -- 11:30 a.m. becomes 690 minutes: (11 * 60) + 30
def time2mins(time):
    time = time.split(":")
    mins =  int(time[0])*60 + int(time[1])
    return mins

def create_flight_graph(flight_data):
    G = {}
    for flight in flight_data:
        ID, origin, dest, dep, arr, cost = flight
        dep = time2mins(dep)
        arr = time2mins(arr)
        dur = arr - dep
        
        if origin not in G:
            G[origin] = {dest: {ID: [cost, dur, dep, arr]}}
        elif dest not in G[origin]:
            G[origin][dest] = {ID: [cost, dur, dep, arr]}
        else:
            G[origin][dest][ID] = [cost, dur, dep, arr]
...
def find_best_flights(flights, origin, destination):
    G = create_flight_graph(all_flights)
    # Start a heap of flight paths:
    # [cost_so_far, dur_so_far, time_so_far, IDs_so_far, path_so_far]
    heap = []
    heapq.heappush(heap, [0,0,0,[],[origin]])
    
    # The heaps store data on every available flight path.
    # Look at every heap to find the best flight.
    while len(heap) > 0:
        
        # Frontier values: pop off the path with the lowest cost so far.
        # If the costs are equal, pop off the path with the lowest duration so far.
        cost_so_far, dur_so_far, time_so_far, IDs_so_far, path_so_far = heapq.heappop(heap)
     
        # The latest flight in the path is locked down,
        # so that's the one to analyze the paths for.
        current_loc = path_so_far[-1]

        # Once we reach the destination, there's no need to continue further.
        if current_loc == destination:
            return IDs_so_far
        
        # Loop through each flight that departs from the current location.
        for interim_dest in G[current_loc]:
            for ID in G[current_loc][interim_dest]:
                cost, dur, dep, arr = G[current_loc][interim_dest][ID]
                
                # Only look at flights that depart after the current flight time.
                # In other words, ignore flights that have already departed.
                if dep > time_so_far:
                    
                    # Update the cost, duration, time, flight path, and flight IDs.
                    # For the updated time, use the arrival time of the latest flight.
                    new_cost = cost_so_far + cost
                    new_dur = dur_so_far + dur
                    new_time = arr
                    new_IDs = IDs_so_far + [ID]
                    new_path = path_so_far + [interim_dest]
                    
                    # Save the latest data to a new heap entry.
                    heapq.heappush(heap, (new_cost, new_dur, new_time,
                                          new_IDs, new_path))            

    # Return None only after looking at all viable flight paths.
    return None
...
# Find the best flights from the origin city to each city in the graph.
def dijkstra(G, origin):
    paths = {}
    for city in set(G): paths[city] = find_best_flights(G, origin, city)
    return paths
